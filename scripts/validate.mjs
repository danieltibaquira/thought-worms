...
const dbSrc = fs.readFileSync('thought-worms.js', 'utf8');
const sandbox = { window: {} };
vm.createContext(sandbox);
vm.runInContext(dbSrc, sandbox, { filename: 'thought-worms.js' });

const rows = sandbox.window.THOUGHT_WORMS;
assert(Array.isArray(rows), 'window.THOUGHT_WORMS must be an array');
assert(rows.length >= 250, `expected at least 250 rows, found ${rows.length}`);
...
for (const [index, row] of rows.entries()) {
  ...
  if (nonText.has(row.category)) {
    assert(row.sourceUrl.startsWith('https://'), `row ${index} non-text source must link`);
  }
...
for (const category of categories) {
  assert(counts[category] > 0, `missing category ${category}`);
}
